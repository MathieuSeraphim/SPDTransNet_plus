...
class SPDFromEEGPreprocessor(BasePreprocessor):
    ...
    def __init__(self, dataset_name: str, **kwargs):
        ...
        self.list_of_standard_signals = None
        self.list_of_combined_signals = None
        self.sequence_of_pairs_to_combine = None
        ...
        self.save_pretrained_features_as_statistic_vectors_flag = None
        self.pretrained_features_lightning_logs_folder = None
        self.pretrained_features_lightning_logs_folder_absolute_directory = None
        self.pretrained_features_batch_size_for_computation = None
        self.pretrained_features_computation_batching_flag = None
        self.number_of_folds = None
        self.correct_non_spd_matrices = None
        self.impose_minimal_eigenvalue = None
    ...
    def parse_initialization_arguments(self, eeg_signals: List[str], labels: List[str],
                                       ...
                                       combined_eeg_signals: Union[List[Any], Tuple[str, str], None] = None,
                                       compute_recording_matrices_no_covariance: bool = False,
                                       pretrained_features_lightning_logs_folder: Union[str, None] = None,
                                       pretrained_features_lightning_logs_folder_absolute_directory: Union[str, None]
                                       = None,
                                       pretrained_features_batch_size_for_computation: Union[int, None] = None,
                                       number_of_folds_if_at_all_relevant: Union[int, None] = None,
                                       correct_non_spd_matrices: bool = False,
                                       impose_minimal_eigenvalue: Union[float, None] = None, random_seed: int = 42,
                                       multiprocessing: bool = False, prior_config_file: Union[str, None] = None):
        assert not self.__initialization_arguments_parsed

        number_of_combined_signals = 0
        combined_signal_names = []
        if combined_eeg_signals is not None:
            number_of_combined_signals = len(combined_eeg_signals)
            for signal_pair in combined_eeg_signals:
                assert len(signal_pair) == 2
                signal1, signal2 = signal_pair
                signal1_decomposed = signal1.split("-")
                signal2_decomposed = signal2.split("-")
                assert len(signal1_decomposed) == len(signal2_decomposed) == 2
                assert signal1_decomposed[0] == signal2_decomposed[0]
                combined_signal_names.append("%s-%s" % (signal2_decomposed[1], signal1_decomposed[1]))
        else:
            combined_eeg_signals = []
        assert len(combined_signal_names) == number_of_combined_signals

        self.number_of_signals = len(eeg_signals) + number_of_combined_signals
        assert self.number_of_signals >= 2
        self.list_of_standard_signals = eeg_signals
        self.list_of_combined_signals = combined_signal_names
        self.sequence_of_pairs_to_combine = combined_eeg_signals
        self.list_of_signals = self.list_of_standard_signals + self.list_of_combined_signals
        ...
        if self.list_of_signal_statistics_to_compute is None:
            self.list_of_signal_statistics_to_compute = []
        ...
        self.pretrained_features_lightning_logs_folder = pretrained_features_lightning_logs_folder
        self.pretrained_features_lightning_logs_folder_absolute_directory =\
            pretrained_features_lightning_logs_folder_absolute_directory
        self.save_pretrained_features_as_statistic_vectors_flag = (self.pretrained_features_lightning_logs_folder is not
                                                                   None)
        self.pretrained_features_batch_size_for_computation = pretrained_features_batch_size_for_computation
        self.number_of_folds = number_of_folds_if_at_all_relevant

        self.pretrained_features_computation_batching_flag = False
        if self.save_pretrained_features_as_statistic_vectors_flag:
            assert self.number_of_folds is not None
            assert self.number_of_folds > 0
            if self.pretrained_features_batch_size_for_computation is not None:
                assert pretrained_features_batch_size_for_computation > 0
                self.pretrained_features_computation_batching_flag = True
    ...
    def per_recording_preprocessing(self, recording_id: int, verbose: bool = True):
        ...
        signal_as_list = []
        for electrode in self.list_of_standard_signals:
        ...
        for electrode1, electrode2 in self.sequence_of_pairs_to_combine:
            assert electrode1 in recording_available_signals and electrode1 in recording_data_keys
            assert electrode2 in recording_available_signals and electrode2 in recording_data_keys
            signal_as_list.append(recording_data[electrode1] - recording_data[electrode2])
        assert len(signal_as_list) == self.number_of_signals
        ...
        for signal_preprocessing_strategy in self.list_of_signal_preprocessing_strategies:
            ...
            assert np.isfinite(preprocessed_signal).all()

            recording_computed_data[signal_preprocessing_strategy] = {}

            channel_signals_list_for_feature_extraction = []
            for channel_id in range(self.number_of_channels):
                ...
                assert np.isfinite(channel_signal).all()

                if self.save_pretrained_features_as_statistic_vectors_flag:
                    channel_signals_list_for_feature_extraction.append(channel_signal)
                ...
                for covariance_estimator in self.list_of_covariance_estimators:
                    ...
                    covariance_matrices = utils.batch_windowed_signal_to_covariance_matrices(
                        ...
                        self.covariance_estimator_extra_arguments_as_dict, self.correct_non_spd_matrices)
            ...
            if self.save_pretrained_features_as_statistic_vectors_flag:
                ...
                if verbose:
                    ...
                    print("Computing epoch feature matrices from a pretrained network, and saving them as individual"
                          "statistic vectors for each subwindow and each fold...")

                device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
                for fold_index in range(self.number_of_folds):

                    if verbose:
                        print("For fold %d..." % fold_index)

                    feature_extractor_model = get_feature_extractor_model(
                        fold_index,
                        self.pretrained_features_lightning_logs_folder,
                        self.pretrained_features_lightning_logs_folder_absolute_directory)
                    feature_extractor_model.to(device)
                    for param in feature_extractor_model.parameters():
                        param.requires_grad = False

                    assert feature_extractor_model.number_of_independent_channels == self.number_of_channels
                    assert feature_extractor_model.number_of_parallel_channels == self.number_of_signals
                    assert feature_extractor_model.signal_length == epoch_size_in_steps

                    epoch_features_size = feature_extractor_model.output_features_length
                    subwindow_features_size = epoch_features_size // self.number_of_subdivisions_per_epoch
                    assert subwindow_features_size == epoch_features_size / self.number_of_subdivisions_per_epoch

                    all_recording_signals_channel_wise = np.stack(channel_signals_list_for_feature_extraction, axis=1)
                    all_recording_epochs_channel_wise = utils.subdivide_signal(all_recording_signals_channel_wise,
                                                                               epoch_size_in_steps)
                    assert all_recording_epochs_channel_wise.shape ==\
                           (self.number_of_signals, self.number_of_channels, number_of_epochs_in_recording,
                            epoch_size_in_steps)

                    # Shape (number_of_epochs_in_recording, number_of_channels, number_of_signals, epoch_size_in_steps)
                    all_recording_epochs_channel_wise = np.transpose(all_recording_epochs_channel_wise, (2, 1, 0, 3))
                    assert all_recording_epochs_channel_wise.shape == \
                           (number_of_epochs_in_recording, self.number_of_channels, self.number_of_signals,
                            epoch_size_in_steps)

                    feature_extractor_input = torch.FloatTensor(all_recording_epochs_channel_wise)

                    batched_output = []
                    if self.pretrained_features_computation_batching_flag:
                        batched_input = feature_extractor_input.split(
                            self.pretrained_features_batch_size_for_computation)
                    else:
                        batched_input = (feature_extractor_input,)

                    for batch in batched_input:
                        batch = batch.to(device)
                        batched_output.append(feature_extractor_model.forward(batch).to("cpu"))
                        del batch
                    del feature_extractor_model
                    recording_learned_features = torch.cat(batched_output, dim=0)

                    # Shape (number_of_epochs_in_recording, number_of_channels, number_of_signals, epoch_features_size)
                    assert recording_learned_features.shape == \
                           (number_of_epochs_in_recording, self.number_of_channels, self.number_of_signals,
                            epoch_features_size)

                    # Shape (number_of_epochs_in_recording, number_of_channels, number_of_signals, number_of_subdivisions_per_epoch, subwindow_features_size)
                    recording_learned_features = recording_learned_features.view(number_of_epochs_in_recording,
                                                                                 self.number_of_channels,
                                                                                 self.number_of_signals,
                                                                                 self.number_of_subdivisions_per_epoch,
                                                                                 subwindow_features_size)

                    for channel_id in range(self.number_of_channels):
                        for feature_vector_index in range(subwindow_features_size):

                            learned_feature_vectors = recording_learned_features[:, channel_id, :, :, feature_vector_index]
                            learned_feature_vectors = learned_feature_vectors.transpose(1, 2).contiguous()

                            assert learned_feature_vectors.shape == (number_of_epochs_in_recording,
                                                                     self.number_of_subdivisions_per_epoch,
                                                                     self.number_of_signals)

                            learned_feature_vectors_key = "learned_vector_%d_for_fold_%d" %\
                                                          (feature_vector_index, fold_index)

                            recording_computed_data[signal_preprocessing_strategy][channel_id]["statistic vectors"][learned_feature_vectors_key] =\
                                learned_feature_vectors
        ...
        for epoch_id in range(number_of_epochs_in_recording):
            ...
            for signal_preprocessing_strategy in self.list_of_signal_preprocessing_strategies:
                ...
                for channel_id in range(self.number_of_channels):
                    ...
                    if self.compute_signal_statistics_flag or self.save_pretrained_features_as_statistic_vectors_flag:
                        ...
                        list_of_statistic_vectors = recording_computed_data[signal_preprocessing_strategy][channel_id]["statistic vectors"].keys()

                        for signal_statistic_vector_type in list_of_statistic_vectors:
...
if __name__ == "__main__":
    main("SPD_matrices_from_EEG_MASS_dataset_PredicAlert_signals_config.yaml")
