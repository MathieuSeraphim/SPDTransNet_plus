...
def get_run_results(hparams_to_track: Dict[str, List[str]], best_run_stats_to_track: Dict[str, List[str]],
    ...
    if rerun_on_validation_set or run_on_test_set:
        ...
        if no_logger:
            trainer_config_dict["logger"] = False
        if "forced_dtype" in trainer_config_modifications_as_dict.keys() and \
                trainer_config_modifications_as_dict["forced_dtype"] == "float64":
    ...
    for runwise_lightning_log_subfolder in runwise_lightning_log_subfolders:
        ...
        try:
            ...
            warnings.warn("Run %s - obtained the following KeyError: %s. Skipping run." % (run_id, str(k)))
            print("Skipping run %s (see warning)." % run_id)
            continue
        ...
        if monitor_validation_value < run_skipping_threshold:
            warnings.warn("Run %s - validation %s value of %4f, under threshold of %.4f. Skipping run."
                          % (run_id, monitor, monitor_validation_value, run_skipping_threshold))
            print("Skipping run %s (see warning)." % run_id)
            continue

        for validation_stat in validation_stats_to_track:
            try:
                stat_validation_events = recorded_events_accumulator.Scalars(
                    "%s/%s" % (validation_stat, valitation_set))
        ...
        if not isfile(hparams_config_file):
            warnings.warn("Run %s - hparams.yaml file not found. Skipping run." % run_id)
            print("Skipping run %s (see warning)." % run_id)
            continue
        ...
        if rerun_on_validation_set or run_on_test_set:
            ...
            if not isdir(checkpoint_folder):
                warnings.warn("Run %s - checkpoints folder not found. Skipping run." % run_id)
                print("Skipping run %s (see warning)." % run_id)
                continue
            ...
            model = get_model_from_checkpoint_file(model_dict, model_checkpoint_file,
                                                   double_precision=double_precision_flag)
        ...
        # Additional operations (none are defined in this version of the code)
...
def get_run_results_from_file(run_analysis_config_name: str, override_batch_size_through_cli: Union[int, None] = None):
    ...
    best_run_stats_to_track_dict = get_best_run_stats_to_track_dict(
        run_analysis_config_dict["best_run_stats_to_track_file"])
    ...
    if override_batch_size_through_cli is not None:
        if "override_batch_size_with_value" in other_inputs_as_dict.keys() and \
                other_inputs_as_dict["override_batch_size_with_value"] is not None:
            warnings.warn("Overriding batch size of %d defined in configuration file with batch size of %d defined in"
                          " CLI." % (other_inputs_as_dict["override_batch_size_with_value"],
                                     override_batch_size_through_cli))
        other_inputs_as_dict["override_batch_size_with_value"] = override_batch_size_through_cli
