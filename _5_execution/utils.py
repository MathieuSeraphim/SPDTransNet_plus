...
def get_model_from_checkpoint_file(model_dict: Dict[str, Any], checkpoint_file: str, no_logging: bool = False,
                                   double_precision: bool = False):
    model = get_model_from_config_dict(model_dict)
    model.setup(None, no_logging=no_logging)
    ...
    if double_precision:
        model = model.double()
        model.setup(None)  # To reset model.example_input_array
    return model
