...
class SPDFromEEGDataset(Dataset):
    ...
    def setup(self, recording_indices: List[int], extra_epochs_on_each_side: int, signal_preprocessing_strategy: str,
        ...
        if rebalance_set_by_oversampling:
            ...
            # In some datasets, elements of a given class may be absent
            classes_in_set = sorted(list(pre_oversampling_elements_per_class_counter.keys()))
            assert len(classes_in_set) <= self.number_of_classes
            if len(classes_in_set) == self.number_of_classes:
                assert classes_in_set == self.label_ids_list
            else:
                for class_id in classes_in_set:
                    assert class_id in self.label_ids_list
            ...
            assert sorted(list(post_oversampling_elements_per_class_counter.keys())) == classes_in_set
            assert self.dataset_length == len(self.dataset_epochs_labels_list) == pre_oversampling_most_common_class_size * len(classes_in_set)
    ...
    def __getitem__(self, item):
        ...
        # shape (number_of_channels, sequences_of_epochs_length, number_of_subdivisions_per_epoch, number_of_signals, subdivision_signal_length)
        ...
        if self.transfer_eeg_epochs_flag:
            ...
            assert sequence_of_epoch_signals_tensor.shape[2] == sequence_of_epoch_matrices_tensor.shape[2]
            assert sequence_of_epoch_signals_tensor.shape[3] == self.matrices_size
