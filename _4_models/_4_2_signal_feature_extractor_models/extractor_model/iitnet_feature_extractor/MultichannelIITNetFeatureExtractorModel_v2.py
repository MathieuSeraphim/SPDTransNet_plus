...
class MultichannelIITNetFeatureExtractorModel_v2(BaseExtractorModel):
    ...
    def __init__(self):
        super(MultichannelIITNetFeatureExtractorModel_v2, self).__init__()
    ...
    def setup(self, number_of_independent_channels: int, number_of_parallel_channels: int, signal_length: int,
        ...
        super(MultichannelIITNetFeatureExtractorModel_v2, self).setup(
        ...
        self.resizing_layer.setup(
            ...
            input_length=self.number_of_output_features * self.vectors_per_smaller_splits * resnet_output_feature_vectors_length,
            output_length=self.number_of_output_features * self.single_output_feature_length,
    ...
    def forward(self, input_signal):
        ...
        assert resnet_output_as_grouped_splits.shape == (
            batch_size,
            self.number_of_independent_channels,
            self.number_of_parallel_channels,
            self.number_of_output_features,
            self.vectors_per_smaller_splits * resnet_output_shape[-1]
        )

        # (batch_size, number_of_independent_channels, number_of_parallel_channels, 1, number_of_output_features * vectors_per_smaller_splits * resnet_output_feature_vectors_length)
        final_resizing_layer_input = resnet_output_as_grouped_splits.view(
            batch_size,
            self.number_of_independent_channels,
            self.number_of_parallel_channels,
            1,  # The number of "regular" channels
            self.number_of_output_features * self.vectors_per_smaller_splits * resnet_output_shape[-1]  # Here, number_of_output_features serves as the number_of_shared_weight_groups dimension for the layer
        )
        ...
        final_resizing_layer_output = self.resizing_layer(final_resizing_layer_input)
...
if __name__ == "__main__":
    ...
    model = MultichannelIITNetFeatureExtractorModel_v2()
