...
class SPDFromEEGDataMultichannelSuccessionLearnedAugmentationReformattingBlock(BaseDataFormattingBlock):

    EXTRA_DIMENSIONS_COMPARED_TO_SINGLE_MATRICES = 2
    INDEX_OF_CHANNEL_DIMENSION = 1

    def __init__(self):
        super(SPDFromEEGDataMultichannelSuccessionLearnedAugmentationReformattingBlock, self).__init__()
        self.__setup_done_flag = False

        self.whitening_layer = MatrixWhiteningLayer()
        self.augmentation_layer = MatrixAugmentationThroughComputedFeaturesLayer()
    ...
    def setup(self, original_matrix_size: int, initial_number_of_matrices_per_epoch: int,  number_of_channels: int,
              feature_extractor_model: Optional[BaseExtractorModel], initial_augmentation_factor: float = 1.,
        ...
        self.whitening_layer.setup(original_matrix_size, operate_whitening,
        ...
        self.operate_matrix_augmentation, augmented_matrix_size = self.augmentation_layer.setup(
            original_matrix_size, feature_extractor_model, initial_augmentation_factor, augmentation_factor_learnable,
            number_of_augmentation_features_optional_parameter)
    ...
    def forward(self, sequence_of_eeg_signals: torch.Tensor, sequence_of_spd_matrices: torch.Tensor,
        ...
        # (batch_size, number_of_channels, sequences_of_epochs_length, number_of_matrices_per_epoch, augmented_matrix_size, augmented_matrix_size)
        if self.operate_matrix_augmentation:
            sequence_of_spd_matrices = self.augmentation_layer(sequence_of_eeg_signals, sequence_of_spd_matrices)
